...
char callsign[9] = "CT1EUS";  // MAX 9 CHARACTERS!!
...
int32_t lat = 0, lon = 0, alt = 0, maxalt = 0, lat_dec = 0, lon_dec =0 ,tslf=0, last_alt=0;
...
void tx_aprs()
{
  ...
  v_accel = (alt - last_alt) / (APRS_TX_INTERVAL * 60.0);
  snprintf(comment, 6, "%4.1f", v_accel);
  ...
  if(tempsensors==1)
  {
    ...
    //"WIDE1", 1, "WIDE2",1,
    "WIDE2", 1, 0, 0,
    "!/%s%sO   /A=%06ld|%s|%s/%s,%d,%i,%i'C,https://twitter.com/spacepeekteam/",
  ...
  else
  {
    ...
    //"WIDE1", 1, "WIDE2",1,
    "WIDE2", 1, 0, 0,
    "!/%s%sO   /A=%06ld|%s|%s/%s,%d,%i,%i,%i'C,https://twitter.com/spacepeekteam/",
    ax25_base91enc(slat, 4, aprs_lat),
    ax25_base91enc(slng, 4, aprs_lon),
